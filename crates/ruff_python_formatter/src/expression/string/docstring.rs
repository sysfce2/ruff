...
impl<'src> CodeExampleDoctest<'src> {
    ...
    ///
    /// If one is found, then a `Left` variant with this doctest is returned.
    /// In this case, the line was kept as part of the code example and callers
    /// should continue trying to add more lines.
    ///
    /// But if one isn't found, then the given line is not part of the code
    /// example and an appropriate action is returned for the caller to
    /// perform.
    fn add_code_line(
        mut self,
        original: InputDocstringLine<'src>,
